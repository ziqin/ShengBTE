...
! Set of routines related to the irreducible wedge of the BZ.
...
contains
  ...
  ! Find k-points in the irreducible wedge. The Brillouin zone is
  ! discretised to product(ngrid) points through Ngrid(:) divisions along each
  ! lattice vector. List(1:Nlist) gives Nlist points in the
  ! irreducible wedge. Nequi(1:Nlist) gives the number of equivalent
  ! points in the whole Brillouin zone.  These k-points are indexed
  ! along the G1 direction first, then the G2 direction and finally
  ! the G3 direction finally. In other words, when the single index is
  ...
  subroutine wedge(Nlist,Nequi,List,ALLEquiList,TypeofSymmetry)
    ...
    do ii=1,nptk
       ...
       ! If iaux=0, the current k point is equivalent to some points
       ! included in AllList, which contains all the equivalent points
       ! of the previous k points.
       if (iaux.eq.1) then
          Nlist=Nlist+1
          List(Nlist)=ii
          Nequi(Nlist)=0
          ...
          do ll=1,Nsymm
             ...
             ! operation on k does not belong to the grid or is
